...
export default function displayUtils() {
  ...
  return {
    ...
    removeToDoByProject(projectName) {
      ...
      let filteredTasks = taskUtilObj.deleteTasksByProjectName(
        currenttaskList,
        projectName
      );
    ...
    addProjectToDiv(projectArr) {
      ...
      let projectParentDivClassList = [
        projectArr[1].toString(),
        "flex",
        "items-center",
        "justify-between",
        "rounded",
      ...
      let projectsFolderImgDivClassList = [
        projectArr[1].toString(),
        "projects-name-image-div",
        "flex",
        "items-center",
        "hover:bg-green-800",
        "hover:font-bold",
      ...
      projectsFolderImgDiv.addEventListener("click", (e) => {
        let projectName = this.getProjectNameById(
          e.target.classList[0].toString()
        );
      ...
      folderImg.setAttribute(
        "src",
        "https://img.icons8.com/ios-glyphs/24/FFFFFF/folder-invoices--v1.png"
      );
      folderImg.setAttribute("alt-text", "folder-invoices--v1");
      let spanProjectName = document.createElement("span");
      let spanProjectNameClassList = [
        projectArr[1].toString(),
        "py-2",
        "px-1",
        "truncate",
      ...
      deleteFolderImg.setAttribute(
        "src",
        "https://img.icons8.com/fluency-systems-filled/24/FFFFFF/filled-trash.png"
      );
      deleteFolderImg.setAttribute("alt-text", "filled-trash");
      let deleteFolderImgClassList = [
        projectArr[1].toString(),
        "hover:w-[28px]",
        "hover:h-[28px]",
        "deleteProjectImg",
    ...
    displayToDoByProject(projectName) {
      ...
      let projectTodoList = taskUtilObj.findWithProject(
        projectName,
        currentTodoList
      );
    ...
    createToDoListDiv(taskInfo) {
      ...
      dueDateImg.setAttribute(
        "src",
        "https://img.icons8.com/officexs/16/calendar.png"
      );
      ...
      let deleteResetBtnDivClassList = [
        "delete-reset",
        "w-[18%]",
        "flex",
        "gap-2",
      ...
      createImg.setAttribute(
        "src",
        "https://img.icons8.com/ios-glyphs/35/FFFFFF/edit--v1.png"
      );
      ...
      deleteIconImg.setAttribute(
        "src",
        "https://img.icons8.com/fluency-systems-filled/35/FFFFFF/filled-trash.png"
      );
    ...
    displayToday() {
      ...
      let projectTodoList = taskUtilObj.tasksWithDate(
        currentTodoList,
        todaysDate
      );
    ...
    editTask(taskId) {
      ...
      for (let i = 0; i < projects.length; i++) {
        if (currentTask.project == projects[i][0]) {
          editselectProjectIp.options[editselectProjectIp.options.length] =
            new Option(projects[i][0], projects[i][0], false, true);
        } else {
          editselectProjectIp.options[editselectProjectIp.options.length] =
            new Option(projects[i][0], projects[i][0], false, false);
        }
      }
      let currentOption = document.querySelector(
        ".editTask-" + currentTask.priority
      );
    ...
    processEditTask(taskId) {
      ...
      let newTask = task(
        editTaskFromData.get("edittaskName"),
        editTaskFromData.get("edittaskDesc"),
        editTaskFromData.get("editdueDate"),
        taskId,
        editprioritySelectIp.value,
        editselectProjectIp.value
      );
      let currentDisplayProject = projectNameSpan.innerText;
      if (currentDisplayProject == "Inbox") {
        this.displayToDoByProject("Inbox");
