  ...
  return {
    handleAddTaskForm() {
      addTaskImg.addEventListener("click", () => {
        ...
        for (let i = 0; i < currentProjects.length; i++) {
          projectInForm.options[projectInForm.options.length] = new Option(
            currentProjects[i][0],
            currentProjects[i][0],
            false,
            true
    ...
    processAddTask() {
      ...
      let currentTask = task(
        addTaskFormData.get("taskName"),
        addTaskFormData.get("taskDesc"),
        addTaskFormData.get("dueDate"),
        getUnixTime(new Date()),
        priorityInForm.value,
        projectInForm.value
      );
      let newTasks = tasks;
